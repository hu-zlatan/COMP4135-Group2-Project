...
No formal implementation meeting minutes were archived in the local workspace for the first prototype checkpoint. This recovered note summarises the result of an early classroom discussion and follow-up implementation alignment that informed the first prototype slice. The in-class discussion itself has no formal record, so the checkpoint summary was compiled from dated artifacts that are present in the repository: the architecture constraints note, the first-pass script plan, and the prototype screenshots created on 15/03/2026.

\begin{itemize}
\item \textbf{Record type:} recovered implementation checkpoint compiled from local artifacts after an unrecorded classroom discussion.
...
\item \textbf{Sources used:} the architecture note and first-pass script plan listed later in the Supporting Record Index, plus prototype screenshots timestamped on 15/03/2026.
...
\item \textbf{Confirmed decisions:} single-battle scope, shared unit controller, deterministic enemy response, explicit card types, button-driven card interaction, and a WR2 scope without campaign or deckbuilding systems.
...
\label{sec:supporting-index}

The file set referenced across WR2 is summarised below. It combines upstream design files with later supporting records so the cited materials can still be located clearly from the repository layout.

\begingroup
\small
\sloppy
\begin{itemize}
\item \path{doc/src/wr1-latex/main.tex}: WR1 report source containing the original requirement, scenario, and system-model baseline inherited by WR2.
\item \path{doc/src/onepager_en.md}: early internal concept note used before the prototype scope was narrowed.
\item \path{doc/src/demo_architecture_constraints.md}: internal architecture-constraint note used to cut the concept down to one battle scene.
\item \path{doc/src/demo_first_pass_scripts.md}: first-pass gameplay script plan for turning the narrowed design into a playable Unity slice.
\item \path{doc/src/project-records/wr2-snapshot-freeze-2026-04-02.md}: freeze note defining the prototype and evidence snapshot used for WR2.
\item \path{prototype-snapshot-verification-2026-04-02.md}: runtime-to-document alignment record used by the Chapter~1 traceability table and the Chapter~4 task priorities.
\item \path{wr2-todo.md}: explicit priority list used as the baseline for the Chapter~4 P0/P1/P2 task summary.
...
\item Merged GitHub PRs \#5, \#6, \#7, \#8, \#9, \#12, \#13, and \#15 in the project repository: remote evidence used by the Chapter~4 task table and the workflow traceability discussion.
\end{itemize}
\endgroup

These records are intentionally lightweight. Their purpose is to stop evidence loss between WR2 and WR3 and to preserve the material that already exists from the prototype cycle.

\section{Traceability and Priority Sources}

The new WR1 to WR2 traceability table in Chapter~1 is derived mainly from the WR1 source, the internal architecture and first-pass script notes, the prototype snapshot verification record, the walkthrough findings, and the WR2 implementation decision log. The prioritised task tables in Chapter~4 combine the WR2 todo list, the evidence register, workflow/process notes, and the merged GitHub PR history listed above. This appendix keeps those source pointers explicit so the new tables do not rely on unspoken evidence.
...
Figure~\ref{fig:wr2-retrospective-board} is included as an explanatory reconstruction because no historical task-board capture survives in the repository. It helps summarise how the closeout work was grouped after the move to GitHub PR workflow. The figure is presented as a retrospective explanation of the closeout stage.
