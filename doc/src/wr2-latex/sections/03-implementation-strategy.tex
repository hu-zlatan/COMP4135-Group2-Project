...
At repository level, the project is split so that the Unity implementation, report sources, and evidence records do not collapse into one mixed file tree. The simplified WR2-relevant top-level layout used during report closeout is:
...
Figure~\ref{fig:module-map} explains code responsibility split instead of runtime event order. Dependencies flow inward toward a small number of managers instead of outward into a large inheritance tree. This matches the goal of a small, traceable prototype.
...
The most important strategic choice was to treat the prototype as implementation evidence for a small tactics slice. That choice explains why the runtime modules are explicit, why the card system stays narrow, and why some features are marked as deferred. For WR2, this keeps the implementation aligned with the report and the current demo scope.
